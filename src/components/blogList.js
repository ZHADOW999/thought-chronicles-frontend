    ...
    return (
        <div>
            {sortedBlogs.map((blog) => {
                ...
                const shortContent = DOMPurify.sanitize(sanitizedContent).slice(0, 100); // Limit sanitized content to 200 chars
                ...
                return (
                    <div key={blog.Blog.id} className="rounded-xl mt-10 bg-white shadow p-5 space-y-5">
                        <div className="items-center flex flex-col md:flex-`row w-full justify-between">
                            <div className="mb-2 md:mb-0 flex flex-row  justify-start self-start h-auto">
                                <ProfilePicture
                                    size={30}
                                    userId={blog.Blog.owner_id}
                                    altText={`User ${blog.Blog.owner_id}'s Profile Picture`}
                                />
                                <div className="ml-5 flex-row justify-center flex  gap-10  md:mt-0">
                                    <p>{blog.Blog.owner.author}</p>
                                    <p className="text-1 flex justify-center items-center">{formatDate(blog.Blog.created_at)}</p>
                                </div>
                            </div>

                            <div className="items-center flex flex-row w-full justify-between" >
                                <div className={`space-y-5 ${images.length > 0 ? "md:[40%] w-[60%]" : "md:w-[80%] w-[100%]"}`}>
                                    <h2 className="sm:text-4xl text-2xl text-text-color font-sans font-black">
                                        {blog.Blog.title}
                                    </h2>
    
                                    {/* Display sanitized content */}
                                    <p className="text-1.5 md:text-2 text-gray-600 flex flex-wrap">
                                        <span className="flex"
                                ...
                                <div className={`${images.length > 0 ? "block" : "hidden"}`}>
                                    {images.length > 0 && (
                                        <img
                                            ...
                                            className=" bg-gray-400 md:w-52 md:h-44 w-24 h-16 "
                                        />
                                    )}
                                </div>
